...
UPDATE_POSITION
    ...
    ADD R2, R2, R1         ; dino_y += velocity
    STRH R2, [R0]

    ; Check if Dino is below ground
    ADD R3, R2, #NORMAL_DINO_H
    CMP R3, #GROUND_Y
    BLT end_update_position

    ; Clamp to ground, reset velocity & acc
    MOV R2, #GROUND_Y - NORMAL_DINO_H
    STRH R2, [R0]
...
UPDATE_VELOCITY
    ...
    LDR R0, =DINO_VELOCITY
    LDRH R1, [R0]
    SXTH R1, R1            ; Sign extend velocity

    LDR R2, =ACC
    LDRH R2, [R2]
    SXTH R2, R2            ; Sign extend acceleration

    ADD R1, R1, R2         ; velocity += acceleration
    STRH R1, [R0]
