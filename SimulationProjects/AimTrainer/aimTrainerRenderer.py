...
print(memory)
CursorX = decode_word(join_dict_values_in_range(memory, 0, 1)) # 00A0 => 160
CursorY = decode_word(join_dict_values_in_range(memory, 2, 3))
OBJ1X = decode_byte(memory.get(4, "00"))
OBJ1Y = decode_byte(memory.get(5, "00"))
OBJ2X = decode_byte(memory.get(6, "00"))
OBJ2Y = decode_byte(memory.get(7, "00"))
OBJ3X = decode_byte(memory.get(8, "00"))
OBJ3Y = decode_byte(memory.get(9, "00"))
...
while running:
    ...
    memory = parse_hex386(file_path)
    try:
        CursorX = decode_word(join_dict_values_in_range(memory, 2, 3))
        CursorY = decode_word(join_dict_values_in_range(memory, 0, 1))
        OBJ1Y = decode_byte(memory.get(4, "00")) * 10
        OBJ1X = decode_byte(memory.get(5, "00")) * 10
        OBJ2Y = decode_byte(memory.get(6, "00")) * 10
        OBJ2X = decode_byte(memory.get(7, "00")) * 10
        OBJ3Y = decode_byte(memory.get(8, "00")) * 10
        OBJ3X = decode_byte(memory.get(9, "00")) * 10
    except Exception as e:
        print(f"Error updating object positions: {e}")

    

    # Draw dartboard-style targets for the objects
    for obj_x, obj_y in [(OBJ1X, OBJ1Y), (OBJ2X, OBJ2Y), (OBJ3X, OBJ3Y)]:
        pygame.draw.circle(screen, (100, 255, 100), (obj_x, obj_y), 10)  # Outer circle (filled)

    # Draw the crosshair for the cursor
    pygame.draw.line(screen, (255, 0, 0), (CursorX - 10, CursorY), (CursorX + 10, CursorY), 2)
    pygame.draw.line(screen, (255, 0, 0), (CursorX, CursorY - 10), (CursorX, CursorY + 10), 2)
    pygame.display.flip()
    clock.tick(30)
